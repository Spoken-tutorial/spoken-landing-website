...
class TutorialRecommender(object):
    ...
    def fit(self, user_x_product, Q=None, latent_features_guess=2, learning_rate=0.0002, steps=50, regularization_penalty=0.02, convergeance_threshold=0.001):
        """
        ...
        return self.__factor_matrix(user_x_product,Q, latent_features_guess, learning_rate, steps, regularization_penalty, convergeance_threshold)
    ...
    def predict_instance(self, row_index):
        ...
        :return:
        """
        print('--',self.P, '--',self.Q.T)
        return numpy.dot(self.P[row_index, :], self.Q.T)
    
    def predict_user(self, user_pr):
        """
        Returns all predictions for a given row
        :param row_index:
        :return:
        ...
        return numpy.dot(self.user_pr, self.Q.T)
    ...
    def __factor_matrix(self, R, Q, K, alpha, steps, beta, error_limit):
        """
        R = user x tutorial matrix
        K = user latent features count (how many features we think the model should derive)
        alpha = learning rate
        ...
        Q = Tutorial x feature matrix. (How strongly a product is associated with a feature)
        ...
        # Generate Q - M x K
        # Use random values to start. Best performance (original)
        # modification - works better with latent data from database
        if not Q:
            ...
            for i in range(len(R)):
                for j in range(len(R[i])):
                    if int(R[i][j]) > 0:
                        ...
                        eij = int(R[i][j]) - numpy.dot(P[i, :], Q[:, j])
        ...
        for i in range(len(R)):
            for j in range(len(R[i])):
                if int(R[i][j]) > 0:
                    ...
                    e = e + pow(int(R[i][j])-numpy.dot(P[i,:],Q[:,j]), 2)
